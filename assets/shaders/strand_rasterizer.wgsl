...
fn rasterize_strands(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    let clip_dim = (cb[1] - cb[0]); // / vec3<f32>(f32(config.screen_width), f32(config.screen_height), 1.0);
    let tm_x = (px_f.x / f32(config.screen_width)) * clip_dim.x + cb[0].x;
    let tm_y = (px_f.y / f32(config.screen_height)) * clip_dim.y + cb[0].y;

    // let light_z = textureLoad(
    //     directional_shadow_textures,
    //     vec2<i32>(i32(tm_x * f32(config.screen_width)), i32(tm_y * f32(config.screen_height))), cascade_index, 0
    // );
